...
The recommended official NIST tools for generating SWID Tags aren't properly documented or not usable in our use case, so we opted to search for other open-source tools.


\paragraph{swid-generator} is a Maven plugin published by Labs64 \cite{repo:swid-generator} that generates SWID Tags for Java projects and is compliant with the above mentioned ISO standard. It needs to be configured in the project's \verb|pom.xml| file.
